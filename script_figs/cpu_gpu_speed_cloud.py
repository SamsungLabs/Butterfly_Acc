...
Speedup_cloud = {
  "Over_GPU": [10.9,  9.0,   4.1,   2.6,   1.9,   9.7,   8.0,   3.6,   2.4, 1.9],
  "Over_CPU": [497.6, 763.2, 677.9, 657.5, 667.5, 525.2, 847.2, 779.2, 761.5, 756]
}
...
def draw_figs(args):
    ...
    speedup_over_gpu = Speedup_cloud["Over_GPU"]
    speedup_over_cpu = Speedup_cloud["Over_CPU"]
    ...
    for i in range(len(label_name)):
        ...
        x_init +=2.2
    ys = []
    for i in range(len(speedup_over_gpu)):
        ys.append(speedup_over_gpu[i])
        ys.append(speedup_over_cpu[i])
    xtick_name = []
    for x in label_name:
        xtick_name.append(x)
    ...
    ax.bar_label(bars, fontsize=font_size)
    bars  = ax.bar(xs[2:], ys[2:], color=[COLORS[0], COLORS[1]] * int(len(ys)/2-1), edgecolor='black', width=barWidth, align='edge')
    ax.bar_label(bars, fontsize=font_size)
    ...
    ax.set_ylabel('Speedup', fontsize=font_size+1)
    ...
    # plt.grid()
    plt.show()
    fig.savefig("CPU_GPU_Speeup_Cloud.pdf", bbox_inches='tight')
