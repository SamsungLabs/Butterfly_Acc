...
def draw_figs(acc, lat):
    ...
    baxes = brokenaxes(ylims=[[53.0,55.6],[62.0,64.2]], hspace=.15, despine=False)
    ...
    print (paratos[:,0])
    print (paratos[:,1])
    baxes.plot(paratos[:,1], paratos[:,0], color=BROWN_DARKER, lw=1.5, label="Pareto Frontier")
    baxes.scatter(lat, acc, fc=BLUE, s=70, ec="white", alpha=0.9)
    baxes.scatter(paratos[:,1], paratos[:,0], fc=BROWN_DARKER, s=70, ec="white", alpha=0.9)
    ...
    # ax.set_ylim([0, 100])
    baxes.set_xlabel('Latency (ms)', fontsize = 11, labelpad=16)
    baxes.set_ylabel('Accuracy (%)', fontsize = 11, labelpad=32)
    baxes.axhline(y=63.7, color=DARK_BLUE, linestyle='-', label="Vanilla Transformer")
    baxes.axhline(y=62.7, color=ORANGE, linestyle='-', label="Accuracy Constraint")
    baxes.legend(prop={'size': 10}) # ncol=len(label_name), loc='lower left', bbox_to_anchor=(0.0, 1.0), 
    ...
    baxes.tick_params('y', labelsize = 10)
    # baxes.yaxis.set_major_formatter(mtick.PercentFormatter())
    baxes.grid()
    plt.show()
    fig.savefig("Codesign_Text.pdf", bbox_inches='tight')
