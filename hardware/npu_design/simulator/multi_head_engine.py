...
class Multi_Head_Engine:
    def __init__(self, head_dim, hidden_dim, num_len, ffn_inner_dim, pr_lt=2, pv_lt=32, pv_ln=8, mac_factor=2, bit_width=8, dram_bw=512, softmax_delay=72):
        ...
        self.pv_qk = (self.pv_lt / self.pc_qk) * (self.num_len / self.hidden_dim)
        ...
        self.coef_bram = Bram(self.hidden_dim * self.ffn_inner_dim // (self.pr_lt * self.pv_lt * self.pc_lt * self.p_head), self.bit_width, self.pr_lt * self.pv_lt * self.pc_lt * self.p_head, "coef_bram")
    ...
    def run_att(self):
        ...
        num_run = int(self.hidden_dim / self.head_dim / self.p_head)
        ...
        bram_data_read_cycles = self.data_bram.read(self.num_len, self.hidden_dim, self.bit_width, self.head_dim, 1) # Read to data bram
        ...
        logging.info("Runtime cost per %d heads takes %d cycles"%(self.p_head, self.run_cycles))
    ...
    def run_lp(self): # Linear projection layer after attention
        ...
        # Data stay, Coef repeat
        for i in range(self.num_len):
            bram_data_read_cycles = self.data_bram.read(1, self.hidden_dim, self.bit_width, self.head_dim, 1) # Read to data bram
            ...
            initial_compute_delay_fc_qk_ce, fc_qk_ce_compute_time = self.qk_ce.run(1, self.hidden_dim/8, self.hidden_dim) # Heigh, Width
            initial_compute_delay_fc_sv_ce, fc_sv_ce_compute_time = self.sv_ce.run(1, self.hidden_dim/8, self.hidden_dim) # Heigh, Width
    ...
    def run_fc1(self):
        ...
        # Data stay, Coef repeat
        for i in range(self.num_len):
            bram_data_read_cycles = self.data_bram.read(1, self.hidden_dim, self.bit_width, self.ffn_inner_dim, 1) # Read to data bram
            ...
            initial_compute_delay_fc_qk_ce, fc_qk_ce_compute_time = self.qk_ce.run(1, self.ffn_inner_dim/8, self.hidden_dim) # Heigh, Width
            initial_compute_delay_fc_sv_ce, fc_sv_ce_compute_time = self.sv_ce.run(1, self.ffn_inner_dim/8, self.hidden_dim) # Heigh, Width
    ...
    def run_fc2(self): # Run the second FC layer in FFN
        ...
        # Data stay, Coef repeat
        for i in range(self.num_len):
            bram_data_read_cycles = self.data_bram.read(1, self.ffn_inner_dim, self.bit_width, self.hidden_dim, 1) # Read to data bram
            ...
            initial_compute_delay_fc_qk_ce, fc_qk_ce_compute_time = self.qk_ce.run(1, self.hidden_dim/8, self.ffn_inner_dim) # Heigh, Width
            initial_compute_delay_fc_sv_ce, fc_sv_ce_compute_time = self.sv_ce.run(1, self.hidden_dim/8, self.ffn_inner_dim) # Heigh, Width
