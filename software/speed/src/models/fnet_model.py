...
from ..configs.fnet_config import Vanilla_FNetConfig


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "google/fnet-base"
_CONFIG_FOR_DOC = "Vanilla_FNetConfig"
    ...
    def __init__(self, config):
        ...
        for i in range(config.num_hidden_layers):
            if self._is_attention_layer(i):
                layer_list.append(BertLayer(config))
            else:
...
class FNetPreTrainedModel(PreTrainedModel):
    ...
    config_class = Vanilla_FNetConfig
...
FNET_START_DOCSTRING = r"""
    ...
    behavior.
    Parameters:
        config ([`Vanilla_FNetConfig`]): Model configuration class with all the parameters of the model.
...
class FNetModel(FNetPreTrainedModel):
    ...
    def forward(
        ...
        if (
            ...
            raise ValueError(
                "The `tpu_short_seq_length` in Vanilla_FNetConfig should be set equal to the sequence length being passed to the model when using TPU optimizations."
            )
