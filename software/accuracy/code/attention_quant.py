...
class Attention_Quant(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        if self.attn_type == "softmax":
            ...
            from fft_attention import FNetBasicFourierTransform
            self.attn = FNetBasicFourierTransform()
        ...
        self.is_quant = False
        for i in range(5):
            (self.att_quantizers).append(Quantizer(forward_number=config["quant_num"], backward_number=config["quant_num"],
    ...
    def forward(self, X, mask):
        ...
        if self.attn_type.startswith("longformer") or self.attn_type.startswith("reformer"):
        ...
        elif self.attn_type.startswith("fft"):
            attn_out = self.attn(X.float())
            if self.is_quant: attn_out = (self.att_quantizers[0])(attn_out)
            attn_out = attn_out[0]
            return attn_out
        else:
            ...
            if self.is_quant: 
                Q = (self.att_quantizers[0])(Q)
                K = (self.att_quantizers[1])(K)
                V = (self.att_quantizers[2])(V)
            with torch.cuda.amp.autocast(enabled = False):
            ...
            if self.is_quant: attn_out = (self.att_quantizers[3])(attn_out)
            attn_out = self.combine_heads(attn_out)

        out = self.ff(attn_out)
        if self.is_quant: out = (self.att_quantizers[4])(out)
