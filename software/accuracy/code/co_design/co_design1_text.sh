...
do
    ...
    do
	    ...
	    output=$(python3 run_tasks.py --model fft --task text  --is_butterfly --hidden_dim_ratio $ratio --transformer_dim $dim --num_layers $layer --dropout_prob 0.1 2>&1 | tee -a co_design1_text.log) 
	    a=$((a + 1))
        done
    done
done
